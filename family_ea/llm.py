...
class TodoOp(BaseModel):
    ...
    due: str = Field(
        default="",
        description="до якого дня зробити, YYYY-MM-DD; порожньо — не міняти; «-» прибрати",
    )
    ...
    remind_on: str = Field(
        default="",
        description="день, коли нагадати про задачу (о 10:00), YYYY-MM-DD: лише коли просять"
        " («нагадай про це в четвер»); порожньо — не міняти (нова задача без дати й проєкту"
        " сама отримує нагадування на завтра); «-» не нагадувати",
    )
...
(«нам треба…») або неясно чия. due — день, до якого треба зробити (YYYY-MM-DD), або \
порожньо: «завтра» → завтрашня дата, «до п'ятниці» → п'ятниця, «цього тижня» → неділя, \
...
завтра о 10:00 (код ставить remind_on, ти не чіпаєш): скажи в reply «нагадаю завтра». \
remind_on став лише на прохання («нагадай про це в четвер», «штовхни мене за тиждень», \
«нагадай Анні завтра про клініки», коли така задача є в контексті: той день, update з її \
id, а не reminder і не нова задача; «нагадай про це» без дня — завтра), «-» — «не нагадуй \
про це». День чи проєкт, дані задачі пізніше, самі знімають нагадування. project для \
...
до зустрічі», «нагадай завтра о 9 купити квіти»); лише коли названо годину чи подію. \
«Нагадай завтра про …» без години про наявну задачу — це remind_on тієї задачі (бот сам \
нагадає о 10:00 з кнопками), не reminder, і годину не вигадуй. at — коли надіслати, ISO \
8601 з offset; \
...
todo, і в reply чесно скажи, що записав як задачу і нагадаєш завтра о 10:00. Коли \
