...
class TodoOp(BaseModel):
    ...
    project: str = Field(
        default="",
        description="create/update: id проєкту з контексту або назва проєкту з цього ж"
        " повідомлення; порожньо — не міняти; «-» прибрати з проєкту",
    )
...
його для нової задачі: «в Калинівку: написати інструкцію», «це по авто» (create з \
project); наявну задачу переносить update з її id і project, або todos в операції над \
проєктом (див. projects); «-» — забрати з проєкту: задачі без проєкту на вебі лежать під \
«Інше», тож «перенеси в Інше», «в інше» — це project «-», а не проєкт. Проєкту з такою \
назвою нема — не вгадуй і не створюй: задачу запиши без проєкту (в «Інше») і спитай у \
reply, чи створити проєкт.
